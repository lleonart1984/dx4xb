...
#include "Basic_Pathtrace.h"
#include "DT_Transmittance.h"
//
...
//		return W * (SampleSkybox(lastX, w));
//#else
//		return W * (SampleSkybox(lastX, w) + SampleLight(w));
//#endif
//
//	float d = tMax - tMin;
//	x += w * tMin;
...
//		if (t >= d)
//		{
//			float3 s = SampleSkybox(lastX, w);
//			//if (scatters > 0)
//			//	return W * ((s.x + s.y + s.z) * 0.333 + SampleLight(w));
//#ifdef NO_DRAW_LIGHT_SOURCE
//			return W * (s + SampleLight(w) * (scatters > 0));
//#else
//			return W * (s + SampleLight(w));
//#endif
//		}
//
