...
bool EvalTraverseUnitarySphere(out float3 x, inout float3 w, float density, float g, float scatteringAlbedo) {
	float3 win = w;
	...
	return GenerateVariablesWithModel(g, scatteringAlbedo, win, density, x, w);
}
...
bool TraverseUnitarySphere(out float3 x, inout float3 w, float density, float g, float scatteringAlbedo) {
...
float3 ComputePath(float3 O, float3 D, inout int complexity)
{
	...
	while (true)
	{
		...
		else
		{
			...
			float _r = volMaterial.Extinction[cmp] * r;
			while (_r > 2)
			{
				float3 _x, _X, _W;
				if (!EvalTraverseUnitarySphere(_x, w, _r, volMaterial.G[cmp], volMaterial.ScatteringAlbedo[cmp]))
					return 0;
				x += _x * r;

				r = MaximalRadius(x, payload.TransformIndex);
				_r = volMaterial.Extinction[cmp] * r;
			}
		}
	}
}
